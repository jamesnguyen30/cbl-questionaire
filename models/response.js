...
Response.createTable = (result)=>{
    sql.query("create table responses(\
        ...
        primary key(id),\
        foreign key (question_id) references questions(id))", (err, res)=>{
            if(err){
...
Response.insert = (newResponse, result)=>{
    sql.query("insert into responses set response=?, question_id =?", 
    [ newResponse.response, newResponse.question_id],
    (err,res)=>{
        if(err){
            console.log("error :", err)
            result(err,null)
...
Response.selectWithQuestionId = (questionId, result)=>{
    sql.query("select * from responses where question_id=?",[questionId], (err, res)=>{
        if(err){
            console.log("error: ", err)
            result(err,null)
        } else {
